...
// shown above it, and the project number (muted, shown before the title — the
// same numbers used in the All Projects list).
window.HOME_ORDER = [
  ...
  // Full-width pull quote. It spans every column, so to avoid empty cells keep it
  // on a row boundary — a multiple of 6 cards lands cleanly at all breakpoints
  // (1 / 2 / 3 columns).
  ...
  // Full-width featured studio update + list (content in updates.js). Like the
  // quote it spans every column, so it's placed on a row boundary (a multiple of
  // 6 cards precede it) to avoid empty cells at the 1 / 2 / 3-column breakpoints.
